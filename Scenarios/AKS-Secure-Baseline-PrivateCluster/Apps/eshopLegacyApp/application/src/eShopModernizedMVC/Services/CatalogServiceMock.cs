...
namespace eShopModernizedMVC.Services
{
    public class CatalogServiceMock : ICatalogService
    {
        ...
        private List<CatalogRegisterUser> catalogRegisterUsers;

        public CatalogServiceMock()
        {
            catalogItems = new List<CatalogItem>(PreconfiguredData.GetPreconfiguredCatalogItems());
            catalogRegisterUsers = new List<CatalogRegisterUser>(PreconfiguredData.GetPreconfiguredCatalogRegisterUsers());
        }
        ...
        {
            return catalogRegisterUsers.FirstOrDefault(x => x.id == id);
        }
        ...
        {
            var maxId = catalogRegisterUsers.Max(i => i.id);
            catalogRegisterUser.id = ++maxId;
            catalogRegisterUsers.Add(catalogRegisterUser);
        }
        ...
        private List<CatalogRegisterUser> ComposeCatalogRegisterUsers(List<CatalogRegisterUser> items)
        {
            var catalogRegisterUsers = PreconfiguredData.GetPreconfiguredCatalogRegisterUsers();
            // var catalogBrands = PreconfiguredData.GetPreconfiguredCatalogBrands();
            items.ForEach(i => i.SetCatalogRegisterUser(catalogRegisterUsers.First(b => b.id == i.CatalogRegisterUserId)));
            // items.ForEach(i => i.CatalogType = catalogTypes.First(b => b.Id == i.CatalogTypeId));
            return items;
            ;
        }
